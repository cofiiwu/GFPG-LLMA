...
#define VERSION     "version 6.14 (2016.10.15)        " \
...
{                               /* --- find item sets recursively */
  ...
  if (rd->mode & FPG_TAIL) {    /* if to use head union tail pruning */
    for (s = rd->map, i = 0; i < fpt->cnt; i++)
      *--s = fpt->heads[i].item;/* collect the tail items */
    r = isr_tail(rd->report, s, (ITEM)(rd->map-s));
  ...
  for (r = 0; i != z; i += fpt->dir) {
    ...
    #ifdef VISITED              /* if to report visited search nodes */
    rd->visited += 1;           /* count current node as visited */
    if ((rd->visited % 10000) == 0) {
      fprintf(stderr, "\r%24"SIZE_FMT, rd->visited);
      fprintf(stderr, "  %24"SIZE_FMT, isr_repcnt(rd->report));
...
{                               /* --- add an item set to the tree */
  ...
  if (*ids < 0) {               /* if there are packed items, */
    ...
    fpt->heads[0].item |= i;    /* compute union of packed items */
...
{                               /* --- find item sets recursively */
  ...
  #endif                        /* abort the recursion */
  if (rd->mode & FPG_TAIL) {    /* if to use head union tail pruning */
    s = rd->map; i = k = 0;     /* get the item set buffer */
    if (fpt->heads[0].item < 0) {
      for (i++, z = 0; z < 16; z++)
        if (fpt->heads[0].item & (1 << z))
          *--s = z;             /* if there are packed items, */
    }                           /* first collect these tail items */
    for ( ; i < fpt->cnt; i++)  /* and then the other tail items */
      *--s = fpt->heads[i].item;
    r = isr_tail(rd->report, s, (ITEM)(rd->map-s));
    if (r) return r;            /* if tail needs no processing, */
  }                             /* abort the recursion */
...
{                               /* --- search for frequent item sets */
  ...
  rd.visited = 0;               /* initialize the search node counter */
  #endif                        /* (rough search complexity measure) */
  ...
  if (mode & FPG_FIM16) {       /* if to use a 16-items machine */
    ...
    fpt->heads[0].item = TA_END;/* create a 16-items machine */
...
{                               /* --- find item sets recursively */
  ...
  #endif                        /* abort the recursion */
  if (rd->mode & FPG_TAIL) {    /* if to use head union tail pruning */
    for (s = rd->map, i = 0; i < cst->cnt; i++)
      *--s = cst->heads[i].item;/* collect the tail items */
    r = isr_tail(rd->report, s, cst->cnt);
    if (r) return r;            /* if tail needs no processing, */
  }                             /* abort the recursion */
...
{                               /* --- find item sets recursively */
  ...
  #endif                        /* abort the recursion */
  if (rd->mode & FPG_TAIL) {    /* if to use head union tail pruning */
    for (map = rd->map, i = 0; i < tdt->cnt; i++)
      *--map = tdt->items[i];   /* collect the tail items */
    r = isr_tail(rd->report, map, tdt->cnt);
    if (r) return r;            /* if tail needs no processing, */
  }                             /* abort the recursion */
